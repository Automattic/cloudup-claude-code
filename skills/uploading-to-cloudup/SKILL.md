...
- Before upload, speak only if safety confirmation is required.
...
Treat uploads like posting a public GitHub comment. Before calling an upload tool, ask for confirmation unless either is true:

- The user explicitly asked to upload this exact image/path in this turn.
- The image was captured/generated by the agent in this turn from a public, unauthenticated URL.

When asking, use one short sentence naming the visible content or path. Example: `Upload this screenshot of the wp-admin dashboard?`

Always stop and ask if the image may contain secrets, personal data, internal/staging URLs, source/config, chat/ticket/customer content, or anything from the user's desktop/clipboard whose origin is unclear.
